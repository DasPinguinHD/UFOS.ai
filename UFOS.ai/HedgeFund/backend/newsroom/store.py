"""In-memory event store for the Financial Newsroom (copied from Global
Monitoring's events.py when insider trades moved here, 2026-09-24 — modules
deliberately keep their own small copies instead of sharing code).

Every collector emits events in the same shape:

    {"id", "category", "title", "summary", "source", "url", "timestamp",
     "lat"/"lon" (unused here, None), "severity"} + collector-specific extras

EventStore keeps the last N events per category (newest first) and a
seen-id set, so a re-polled item isn't stored twice.
"""
