...
# If another backend is already listening on the websocket port, exit early to avoid
# attempting to start a second server and creating a bind conflict.
try {
	...
	if ($tnc) {
		...
		if ($res -and $res.TcpTestSucceeded) {
			Write-Host "Backend listener already present on 127.0.0.1:8765; exiting.";
			exit 0
		}
	}
	else {
