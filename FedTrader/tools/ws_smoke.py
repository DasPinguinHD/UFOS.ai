...
Usage: python ws_smoke.py --uri ws://127.0.0.1:8765 --timeout 5
Exits 0 on success (connect + optional ping), 1 on connect error, 2 on missing module.
"""
import argparse
import asyncio
import sys
...
async def run(uri: str, timeout: int) -> int:
    try:
        ...
        async with websockets.connect(uri) as ws:
            print("CONNECTED")
            # try to send a ping-like text message
            try:
                ...
                return 1
            # wait a small moment for any response
            try:
